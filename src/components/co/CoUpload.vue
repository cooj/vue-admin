...
const initData = () => {
    let imgArr = props.modelValue.split(props.chat)
...
const onImgChange = async (file: UploadFile, files: UploadFiles) => {
    ...
    const res = await ApiCommon.upload({ image: file.raw as File })
...
const onDeleteFile = async () => {
    ...
    // 删除已上传的图片
    Promise.all([
        defData.deleteList.map(item => ApiCommon.delImg({ src: item })),
    ])
    defData.deleteList = []
}
...
// 数组转成字符串形式
const listToStr = (arr: any[], t = props.chat) => {
    let url = ''
